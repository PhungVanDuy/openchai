...
from openchai.process_data.chatml_processor import ChatMLConvoProcessor
from openchai.process_data.utils import get_raw_dataset


class BaseProcessor:
    def __init__(self, tokenizer, max_length, output_format):
...
class ChatMLDataset:
    def __init__(self, tokenizer, dataset_name, val_set_size=0.01, sequence_len=4096):
        self.input_format = "chatml"
...
def prepare_chatml_dataset(cfg, tokenizer):
    ...
    for data in cfg.datasets:
        dataset = ChatMLDataset(tokenizer, data.path, cfg.val_set_size, cfg.sequence_len)
